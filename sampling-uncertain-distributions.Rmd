---
title: "Meta-analysis of the SARS-CoV-2 serial interval and the impact of parameter uncertainty on the COVID-19 reproduction number - Supplementary 2"
output: 
...
knit: (function(inputFile, encoding,...) {
  rmarkdown::render(
    inputFile,
    encoding = encoding,
    output_dir = here::here("output"), output_file=paste0('serial-intervals-suppl-2-',Sys.Date(),'.pdf'))
  })
---
...
# Sampling from uncertain distributions
...
```{r}
here::i_am("sampling-uncertain-distributions.Rmd")
...
A normally distributed variable $x$ with an expected value $\mu$ and a standard deviation $\sigma$ is sampled $n$ times. the set of $x_n$ observations has a mean of $\overline{x}$ and observed variance of $S_n^2$,  the sampling distribution of the variance can be shown to be a Chi-squared distribution^2^ with $n-1$ degrees of freedom. Given that the Chi-squared distribution is a particular form of a Gamma distribution (here parameterised with shape, $\alpha$ and rate, $\beta$) and given the definition of the Nagakami-m distribution^3^, the following holds:

$$
\begin{aligned}
(n-1)S^2_n/\sigma^2=\frac{1}{\sigma^2}\sum_i(x_i-\overline{x})^2\sim\tilde{\chi}^2_{n-1} \\
\tilde{\chi}^2_{n-1} = \text{Gamma}\Big(\frac{n-1}{2},\frac{1}{2}\Big)\\
X\sim\text{Gamma}(\alpha,\beta) \implies 
yX\sim\text{Gamma}(\alpha,\beta/y) \\
...
X \sim \text{Gamma}(\alpha,\beta) \implies \sqrt{X} \sim \text{Nakagami}( m, \Omega)\\
...
The main purpose of this approach is to generate a representative sample set from uncertainly specified parameterised distributions such that they can be combined. To test this we investigate a list of published studies that give estimates of the serial interval of SARS-CoV-2 as a parameterised distribution (see Table S2.1).

*Table S2.1: A set of uncertainly specified parameterised distributions extracted from the SARS-CoV-2 literature.*

```{r}


input = serialIntervals %>% filter(estimate_type == "serial interval" & assumed_distribution %in% c("gamma","lnorm","norm")) %>%
  select(
    label,
    N = sample_size,
...
inputSumm %>% select(source,dist,N,param,summary) %>% group_by(source,dist,N) %>% standardPrintOutput::saveTable(defaultFontSize = 7)
```
...
*Table S2.2: The comparison between original uncertain distributions, and the result of aggregating samples generated using the procedure described in this paper. A good agreement is shown when source distributions are reasonably well constrained to begin with* 
...
bind_rows(
  ...
  standardPrintOutput::saveTable(defaultFontSize = 7)
```
...
4. Douillet PL. Sampling distribution of the variance. In: Proceedings of the 2009 Winter Simulation Conference (WSC) [Internet]. Austin, TX, USA: IEEE; 2009 [cited 2021 Jun 29]. p. 403–14. Available from: http://ieeexplore.ieee.org/document/5429351/
5. O’Neill B. Some Useful Moment Results in Sampling Problems. The American Statistician [Internet]. 2014 Oct 2 [cited 2021 Jul 3];68(4):282–96. Available from: https://doi.org/10.1080/00031305.2014.966589


### Serial interval source citations

```{r results="asis"}
tmp = serialIntervals %>% filter(estimate_type == "serial interval" & assumed_distribution %in% c("gamma","lnorm","norm")) %>% select(label,source) %>% distinct() %>% mutate(source = paste0(label,"\n  ~ ",source)) %>% pull(source)
