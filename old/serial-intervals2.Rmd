...
here::i_am("serial-intervals2.Rmd")
```
...
si3 = SerialIntervalProvider$fromFF100(dpc, dists=c("gamma","norm","lnorm","weibull"))

# si3$dfit$plot(xlim=c(-7,10))#+#guides(fill="none",colour="none")+
...
si1 = SerialIntervalProvider$resampledSerialInterval(dpc, resamples=resamples, dists=c("gamma","norm","lnorm","weibull"))
...
<!-- devtools::load_all("~/Git/uk-covid-datatools/") -->
<!-- ukcovidtools::reload() -->
<!-- si1 = SerialIntervalProvider$resampledSerialInterval(dpc, resamples=resamples, dists=c("gamma","norm","weibull")) -->
<!-- resampleSIresult = si1$getSummary() %>% summarise(label = sprintf("mean %1.1f (95%% CrI %1.1f\U2013%1.1f) and SD %1.1f (95%% CrI %1.1f\U2013%1.1f) days",meanOfMean,minOfMean,maxOfMean,meanOfSd,minOfSd,maxOfSd)) -->

<!-- si1$dfit$plot(xlim = c(-7,21)) -->
...
si1$getBasicConfig(quick = FALSE)$si_sample %>% write.table(here::here("output/resampled-truncated-empirical-si-sample.txt"))
```
...
latestDataFile = dpc$datasets$downloadAndUntar(id = "BE-OUTBREAK-PREPARED",url = 
  #"https://github.com/beoutbreakprepared/nCoV2019/raw/master/latest_data/latestdata.tar.gz", pattern = "latestdata.csv")
  "https://github.com/beoutbreakprepared/nCoV2019/raw/d8a68075fd17707fc8f861e93f775b6ec6e0158b/latest_data/latestdata.tar.gz", pattern = "latestdata.csv")
...
fitdistrplus::cdfcompcens(bopFit$distFits,plotstyle = "ggplot")
fitdistrplus::ppcompcens(bopFit$distFits,plotstyle = "ggplot")
...
#fig6 %>% standardPrintOutput::saveSixthPageFigure(here::here("output/Fig6_ff100Incubation"))
...
incubFit = incubFit$filterModels(aic == min(aic))
...
ukcases = ukts %>% filter(statistic=="case") %>% tsp$logIncidenceStats(smoothingWindow = 7)

siImpact = dpc$datasets$getHashCached(object=ukcases, params = list(siDates, meanlim,sdlim), operation = "SERIAL-INTERVAL-IMPACT", nocache=TRUE, orElse = function(...) {
...
CHESSClean = CHESS %>% chp$chessAdmissionSubset(updatedWithin = 21)
#plots=list()
#tables=list()
...
# onsetFit = DistributionFit$new(distributions = c("lnorm","gamma","weibull"))$fromUncensoredData(rawOnset, valueExpr = time, truncate = TRUE, bootstraps = 100)
...
infectionToObservationFit = DistributionFit$new(distributions = c("lnorm","gamma","weibull"))
...
ukts3 = ukts2 %>%
  tsp$logIncidenceStats(valueVar = Deconv.value, growthRateWindow = 14, smoothingWindow = 7) %>%
...
incidence = ukts6 %>% tsp$plotIncidenceQuantiles(colour=subgroup,dates = c("2020-03-01","2020-07-01"),window=7) + facet_wrap(vars(statistic))+
  scale_color_brewer(palette = "Dark2")

rtComp = ukts5 %>% tsp$plotRt(colour = subgroup,dates = c("2020-03-01","2020-07-01"), rtlim=c(0.5,1.75), ribbons = FALSE)+facet_wrap(vars(statistic))+
  scale_color_brewer(palette = "Dark2")
...
```{r}
si3$dfit$printDistributionDetail() %>% ungroup() %>% select(N=n, AIC=aic, Distribution, `Parameter / Moment` = param, `Mean ± SD (95% CI)`) %>% group_by(Distribution, AIC, N) %>% arrange(AIC) %>% standardPrintOutput::saveTable(here::here("output/TableS1_ParametersFF100SerialInterval"))
```
...
```{r}
si1$dfit$printDistributionDetail() %>% ungroup() %>% mutate(AIC = sprintf("%1.1f",aic)) %>% select(N=n, AIC, Distribution, `Parameter / Moment` = param, `Mean ± SD (95% CI)`) %>% group_by(Distribution, AIC, N) %>% arrange(AIC) %>% standardPrintOutput::saveTable(here::here("output/TableS2_ParametersResampledSerialInterval"))
```
...
## `r cap$stab("chess-trusts","The NHS trusts in the CHESS dataset who report all admissions")`

```{r}
CHESSClean %>% group_by(Trust = trustname) %>% summarise(Patients = n())
```
