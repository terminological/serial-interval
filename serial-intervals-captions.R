...
.index$fig("incub-period","Incubation period distributions reconstructed from Open COVID-19 Data Working Group and from FF100 data. Histogram data is not shown as the interval censored data cannot be plotted in this form")
...
.index$fig("si-impact","Time varying reproduction number given various assumptions on the serial interval mean and standard deviation. The blue points show the central estimate of serial intervals from the literature, whereas the coloured error bars show the mean and standard deviation of the 3 serial interval (green, violet, magenta) and 1 generation interval (orange) estimates presented in this paper. Contours show the *R~t~* estimate for that combination of mean and standard deviation serial interval. The four panels represent the 4 different time points investigated.")
...
.index$sfig("meta-analysis","Forest plot for serial interval studies for A, the mean and B, the standard deviation of the serial interval, using the normal mixture random effect model, and from studies identified in the literature which assume a Gamma distributed serial interval")
.index$stab("dfit-serial-interval-resample","Parametererised serial interval distributions from resampling the literature. Gamma and Negative Binomial estimates are from data truncated at zero. AIC estimates are not comparable to those for Normal distribution which is fitting all data, including negative serial intervals, and hence has a lower mean.")
...
#.index$sfig("observation-interval","Time intervals between infector-infectee observations")
